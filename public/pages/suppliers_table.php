            ...
            <div id="dashboard_content_conteiner">
                ...
                <div class="dashboard_content">
                    <div id="dashboard_content_form">
                        <div class="row">
                            <div class="column">
                                <h2 class="textTopList"><i class="bi bi-list-task"></i> Supplier List</h2>
                                <div class="containerDashboardTable">
                                    <table id="userTable" class="dashboard_table">
                                        ...
                                        <tbody>
                                            <?php
                                                $stmt = $conn->query("SELECT id, supplier_name, supplier_location, email, created_at, updated_at FROM supplier ORDER BY id DESC ");
                                                while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                                                    echo "<tr>";
                                                    echo "<td>" . htmlspecialchars($row['id']) . "</td>";
                                                    echo "<td>" . htmlspecialchars($row['supplier_name']) . "</td>";
                                                    echo "<td>" . htmlspecialchars($row['supplier_location']) . "</td>";
                                                    echo "<td>" . htmlspecialchars($row['email']) . "</td>";
                                                    echo "<td>" . htmlspecialchars($row['created_at']) . "</td>";
                                                    echo "<td>" . htmlspecialchars($row['updated_at']) . "</td>";
                                                    $safeId    = htmlspecialchars($row['id'],         ENT_QUOTES);
                                                    $safeName = htmlspecialchars($row['supplier_name'], ENT_QUOTES);
                                                    $safeLocation = htmlspecialchars($row['supplier_location'], ENT_QUOTES);
                                                    $safeEmail = htmlspecialchars($row['email'], ENT_QUOTES);
                                                    echo "<td>
                                                                <button class='editBtn'
                                                                    data-id='{$safeId}'
                                                                    data-product='{$safeName}'
                                                                    data-codigo='{$safeLocation}'
                                                                    data-commentproduct='{$safeEmail}'>
