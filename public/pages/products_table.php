            ...
            <div id="dashboard_content_conteiner">
                ...
                <div class="dashboard_content">
                    <div id="dashboard_content_form">
                        <div class="row">
                            <div class="column">
                                <h2 class="textTopList"><i class="bi bi-list-task"></i> Product List</h2>
                                <div class="containerDashboardTable">
                                    <table id="userTable" class="dashboard_table">
                                        ...
                                        <tbody>
                                            <?php
                                                $stmt = $conn->query("SELECT id, product_name, codigo, comment, created_at, updated_at FROM products ORDER BY id DESC ");
                                                while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                                                    ...
                                                    $safeProduct = htmlspecialchars($row['product_name'], ENT_QUOTES);
                                                    $sacodigo = htmlspecialchars($row['codigo'], ENT_QUOTES);
                                                    $sacommentProduct = htmlspecialchars($row['comment'], ENT_QUOTES);
                                                    echo "<td>
                                                                <button class='editBtn'
                                                                    data-id='{$safeId}'
                                                                    data-product='{$safeProduct}'
                                                                    data-codigo='{$sacodigo}'
                                                                    data-commentproduct='{$sacommentProduct}'>
